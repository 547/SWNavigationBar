...
extension UINavigationBar:WRAwakeProtocol
{
    ...
    private func getCustomBackgroundImageView() -> UIImageView?{
        if let backgroundImageView = backgroundImageView{
            return backgroundImageView
        }else{
            guard let superViewOfnewBackgroundImageView = subviews.first else { return nil }
            setBackgroundImage(UIImage(), for: UIBarMetrics.default)
            let statusBarHeight = UIApplication.shared.statusBarFrame.height
            let newBackgroundImageView = UIImageView.init()
            backgroundImageView = newBackgroundImageView
            newBackgroundImageView.isUserInteractionEnabled = false
            newBackgroundImageView.autoresizingMask = UIView.AutoresizingMask.flexibleWidth
            newBackgroundImageView.translatesAutoresizingMaskIntoConstraints = false
            superViewOfnewBackgroundImageView.insertSubview(newBackgroundImageView, at: 0)
            let leading = NSLayoutConstraint.init(item: newBackgroundImageView, attribute: .leading, relatedBy: .equal, toItem: superViewOfnewBackgroundImageView, attribute: .leading, multiplier: 1, constant: 0)
            let top = NSLayoutConstraint.init(item: newBackgroundImageView, attribute: .top, relatedBy: .equal, toItem: superViewOfnewBackgroundImageView, attribute: .top, multiplier: 1, constant: -statusBarHeight)
            let trailing = NSLayoutConstraint.init(item: newBackgroundImageView, attribute: .trailing, relatedBy: .equal, toItem: superViewOfnewBackgroundImageView, attribute: .trailing, multiplier: 1, constant: 0)
            let bottom = NSLayoutConstraint.init(item: newBackgroundImageView, attribute: .bottom, relatedBy: .equal, toItem: superViewOfnewBackgroundImageView, attribute: .bottom, multiplier: 1, constant: 0)
            leading.isActive = true
            top.isActive = true
            trailing.isActive = true
            bottom.isActive = true
            superViewOfnewBackgroundImageView.addConstraints([leading,top,trailing,bottom])
            return newBackgroundImageView
        }
    }
    ...
    {
        let imageView = getCustomBackgroundImageView()
...
//        if let fromBarBackgroundColor = fromVC?.navBarBarTintColor, let toBarBackgroundColor = toVC?.navBarBarTintColor {
//            let newBarBackgroundColor  = WRNavigationBar.middleColor(fromColor: fromBarBackgroundColor, toColor: toBarBackgroundColor, percent: progress)
//            setNeedsNavigationBarUpdate(barTintColor: newBarBackgroundColor)
//        }else if let fromBarBackgroundColor = fromVC?.navBarBarTintColor{
//            if let toBarBackgroundImage = toVC?.navBarBackgroundImage {
//
//            }else{
//                let newBarBackgroundColor  = WRNavigationBar.middleColor(fromColor: fromBarBackgroundColor, toColor: fromBarBackgroundColor, percent: progress)
//                setNeedsNavigationBarUpdate(barTintColor: newBarBackgroundColor)
//            }
//        }else if let toBarBackgroundColor = toVC?.navBarBarTintColor{
//            if let fromBarBackgroundImage = fromVC?.navBarBackgroundImage {
//
//            }else{
//                let newBarBackgroundColor  = WRNavigationBar.middleColor(fromColor: toBarBackgroundColor, toColor: toBarBackgroundColor, percent: progress)
//                setNeedsNavigationBarUpdate(barTintColor: newBarBackgroundColor)
//            }
//        }
...
extension UIViewController: WRAwakeProtocol
{
    ...
    var navBarBackgroundImage: UIImage?
    {
        ...
        set {
            let newImage = newValue ?? UIImage()
            objc_setAssociatedObject(self, &AssociatedKeys.navBarBackgroundImage, newImage, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
//                navBar.wr_setBackgroundImage(image: newValue!)
                ...
                if canUpdateNavBarBarTintColorOrBackgroundAlpha == true {
                    navigationController?.setNeedsNavigationBarUpdate(backgroundImage: newImage)
                }
            }
        }
    }
    ...
    var navBarBarTintColor: UIColor? {
        ...
        set {
            let newColor = newValue ?? WRNavigationBar.defaultNavBarBarTintColor
            objc_setAssociatedObject(self, &AssociatedKeys.navBarBarTintColor, newColor, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
//                navBar.wr_setBackgroundColor(color: newValue)
                ...
                if canUpdateNavBarBarTintColorOrBackgroundAlpha == true {
                    navigationController?.setNeedsNavigationBarUpdate(barTintColor: newColor)
                }
            }
        }
    }
...
//                navBar.titleTextAttributes = [NSAttributedStringKey.foregroundColor:newValue]
    ...
    private func canUpdateNavigationBar() -> Bool
    {
        ...
        let frameWhenOpaqueTabbar = CGRect(x: 0, y: 0, width: WRNavigationBar.screenWidth(), height: WRNavigationBar.screenHeight() - WRNavigationBar.tabBarHeight())
        
        // 蝙蝠🦇
        let isBat = viewFrame.equalTo(maxFrame) || viewFrame.equalTo(middleFrame) || viewFrame.equalTo(minFrame) || viewFrame.equalTo(frameWhenOpaqueTabbar)
        if self.navigationController != nil && isBat == true, (self.navigationController?.viewControllers.contains(self) ?? false){
            return true
        } else {
            return false
        }
    }
    func updateNavigationBarDisplayStyle() -> () {
        if let navBarBgImage = navBarBackgroundImage {
            navigationController?.setNeedsNavigationBarUpdate(backgroundImage: navBarBgImage)
        }else if let navBarBarTintColor = navBarBarTintColor {
            navigationController?.setNeedsNavigationBarUpdate(barTintColor: navBarBarTintColor)
        }else if let defaultNavBarBgImage = WRNavigationBar.defaultNavBarBackgroundImage {
            navigationController?.setNeedsNavigationBarUpdate(backgroundImage: defaultNavBarBgImage)
        }else{
            navigationController?.setNeedsNavigationBarUpdate(barTintColor: WRNavigationBar.defaultNavBarBarTintColor)
        }
